...
class GUI:
    ...
    def _setup_ui(self):
        ...
        ttk.Button(file_btn_frame, text=_("添加文件"), command=self._add_files).pack(
            fill=tk.X
        )
        ttk.Button(file_btn_frame, text=_("清空列表"), command=self._clear_files).pack(
            fill=tk.X
        )
        ...
        ttk.Entry(res_frame, textvariable=self.res_x, width=10).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Label(res_frame, text=_("分辨率 Y:")).pack(side=tk.LEFT)
        self.res_y = tk.IntVar(value=1000)
        ttk.Entry(res_frame, textvariable=self.res_y, width=10).pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Entry(font_frame, textvariable=self.font_name).pack(
        ...
        ttk.Checkbutton(
            opt_frame, text=_("不覆盖已有文件"), variable=self.no_overwrite
        ).pack(side=tk.LEFT)
        ...
        ttk.Button(out_frame, text=_("选择目录"), command=self._select_out_dir).pack(
            side=tk.RIGHT
        )
        ...
        self.convert_btn = ttk.Button(
            main_frame, text=_("开始转换"), command=self._start_conversion
        )
    ...
    def _start_conversion(self):
            ...
            try:
                ...
                if self.output_directory:
                    out_path = os.path.join(
                        self.output_directory, os.path.basename(file_path) + ".ass"
                    )
                else:
        ...
        self._log(
            _("所有任务完成。成功: {}, 失败: {}").format(
                success_count, len(self.queue) - success_count
            )
        )
