...
class Annotations2Sub():
    def __init__(self,string,Title='默认文件',PlayResX=1920,PlayResY=1080,Zoom=1):
        self.SimpeAssTools.info.change(Title,PlayResX,PlayResY)
        self.SimpeAssTools.style.change('Defaults',ScaleX=PlayResX/1280*100*Zoom,ScaleY=PlayResY/720*100*Zoom)
        self.xml = xml.etree.ElementTree.fromstring(string)
        self._convert(self.xml) 
        self.SimpeAssTools.save(Title)
    ...
    def _convert(self,string):
        for each in self.xml.find('annotations').findall('annotation'):
            ...
            #提交
            self.SimpeAssTools.event.add(self,Start,End,self.SimpeAssTools.tab_helper(Text,PrimaryColour,BackColour,x,y))

    class SimpeAssTools(object):
        def save(self,file_name):
            with open(file_name+'.ass', 'w',encoding='utf-8') as f:
                f.write(self.info.dump(self))
                f.write(self.style.dump(self))
                f.write(self.event.dump(self))
