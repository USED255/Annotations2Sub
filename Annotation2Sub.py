...
class Annotations2Sub():
    def __init__(self,string,Title='默认文件',PlayResX=1920,PlayResY=1080,Zoom=1):
        ...
        self._save(file_name=Title)

    def _convert(self,string,Title='默认文件',PlayResX=1920,PlayResY=1080,Zoom=1):
        ...
        for each in self.xml.find('annotations').findall('annotation'):
            ...
            #提交
            self.event.add(Start=Start,End=End,Name=Name,Text=self._tab_helper(Text=Text,PrimaryColour=PrimaryColour,BackColour=BackColour,x=x,y=y,Zoom=Zoom))
    ...
    def _tab_helper(self,Text,PrimaryColour=None,BackColour=None,x=None,y=None,Zoom=1):
        pos = "\\pos({},{})".format(str(int(float(x)*10/Zoom)),str(int(float(y)*10/Zoom)))
        c1 = '\c'+PrimaryColour
        c2 = '\\3c'+BackColour
        tab = r'\an7' + pos + c1 +c2
        Text = "{%s}" % tab + Text
